...
def tests_were_executed_in_order(result, order):
    ...
    if len(lines_with_tests_names) != len(order):
        raise Exception(f"Can't check order, unequal lengths.\nExpected: {order}\nGot: {lines_with_tests_names}")
    ...
    for line in lines_with_tests_names:
        if test_file_name in line:
            if f"{number}/{of}" in line:
                ...
                raise Exception(f"Wrong (or missing) number ({number}/{of}) in line: '{line}'")
...
def __check_test_execution(verdict, result, test_file_name):
    tests_names_with_verdicts = [x for x in result.stdout.splitlines()
                                 if "Running test" in x or "Verdict:" in x]

    index_of_line_with_test = __index_of_test_in_lines(tests_names_with_verdicts,
                                                       test_file_name)

    __check_test_vertdict_exists(tests_names_with_verdicts,
                                 index_of_line_with_test,
                                 test_file_name)

    assert f"Verdict: {verdict}" in tests_names_with_verdicts[index_of_line_with_test + 1]
